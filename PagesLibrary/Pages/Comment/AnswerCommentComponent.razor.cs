...
{
    public partial class AnswerCommentComponent : IDisposable
    {
        ...
        private async Task SetReaction(int id, int value)
        {
            try
            {
                var result = await _reaction.SetReaction(id, value, Memy.Shared.Helper.MyEnums.TypOfReaction.AnswerComment);

                if (result != null)
                {
                    _popUp.ShowToats("Reakcja dodana", "Dodawanie reakcji", PopupLevel.Level.Success, 1);
                    AnswerCommentModel.Reaction = result;
                }
                else
                {
