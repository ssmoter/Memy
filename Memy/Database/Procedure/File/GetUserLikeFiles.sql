CREATE PROCEDURE GetUserLikeFiles
@token NVARCHAR(MAX),
@start INT = 0,
@max INT = 10,
@orderTyp int = 0
...
--pobranie czy dany użytkownik dodał reakcje i jaką
	...
	JOIN FilerReaction ON FileModel.Id = FilerReaction.FileSimpleId
	WHERE FileModel.Banned = 0
	AND FilerReaction.UserId = @userId AND (FilerReaction.Value != 0 )
